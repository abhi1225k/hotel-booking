...
const AllRooms = () => {
  ...
  return (
    ...
    justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 bg-gray-100'>
      <div>
        <div className='flex flex-col items-start text-left'>
            ...
            max-w-174'>Take advantage of our limited time offers and special packages to enchace your stay and create unforgettable memories.</p>
        </div>
         ...
         {roomsDummyData.map((room) => (
            ...
            {/*room price*/}
            <p className='text-x1 font-medium text-gray-900'>₹{room.pricePerNight} /night</p>
            </div>
            </div>
         ))}
      ...
      <div className={`${openFilters ?'h-auto' : "h-0 lg:h-auto"} overflow-hidden transition-all duration-700`}>
        <div className='px-5 pt-5'>
          <p className='font-medium text-gray-900 pb-2'>Popular filters</p>
          {roomTypes.map((room, index) => (
            <CheckBox key={index} label={room} />
          ))}
        </div>
        <div className='px-5 pt-5'>
          <p className='font-medium text-gray-900 pb-2'>Price Range</p>
          {priceRanges.map((range, index) => (
            <CheckBox key={index} label={`$ ${range}`} />
          ))}
        </div>
        <div className='px-5 pt-5 pb-7'>
          <p className='font-medium text-gray-900 pb-2'>Sort By</p>
